// nova-scraper.js
import { InsertVehicle } from '@shared/schema'; // Assuming this path is correct in your project
...
async function getAllVehicleUrls(inventoryUrl) {
  ...
  do {
    ...
    try {
      ...
      if (!foundNextPage) {
        currentPageUrl = ''; // No more pages or next page already visited/invalid
  ...
  try {
    ...
    if (titleMatch) {
        ...
        $('.detail-value, .spec-value, .vehicle-details li, .vehicle-info li, .specs-table td').each((_, elem) => {
            ...
            if (text.toLowerCase().includes('year:') && !year) year = parseInt(text.split(':')[1]?.trim(), 10);
        });
    }
    ...
    const vehicle = {
      ...
      mileage,
      vin,
      location: 'McLean, VA', // Hardcoded as per your original file
      zipCode: '22102', // Hardcoded as per your original file
      dealershipId,
      images,
    ...
    console.log(`Successfully scraped Nova Autoland vehicle: ${year} ${make} ${model} (VIN: ${vin})`);
    console.log(`  Price: $${price}, Mileage: ${mileage}, Images: ${images.length}`);
    return vehicle;

  } catch (error) {
    console.error(`Error processing Nova Autoland vehicle ${url}: ${error.message}`);
    return null;
  }
}
    ...
    $('script[type="application/ld+json"]').each((_, element) => {
      ...
      try {
        ...
        if (json.offers && json.offers.itemOffered && json.offers.itemOffered.additionalProperty) {
            const carfaxProperty = json.offers.itemOffered.additionalProperty.find(prop => prop.name === 'Carfax Report URL' && prop.value);
