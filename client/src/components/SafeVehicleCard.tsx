...
export default function SafeVehicleCard({ vehicle, onClick }: VehicleCardProps) {
  // Safe getters for all properties to prevent rendering errors
  const safeGet = {
    title: () => {
      try {
        return vehicle.title || 'Untitled Vehicle';
      } catch (e) {
    ...
    mileage: () => {
      try {
        return typeof vehicle.mileage === 'number' ? vehicle.mileage : 0;
      } catch (e) {
        return 0;
      }
    },
    imageUrl: () => {
      try {
        ...
        if (vehicle.make === 'Honda' && vehicle.model === 'Pilot') {
          return 'https://cdn05.carsforsale.com/00ec2e05785d5b6cdc7fbb8c6f5db718/1280x960/2005-honda-pilot-ex-awd-4dr-suv.jpg';
        }
    ...
    dealershipName: () => {
      try {
        return vehicle.dealership?.name || 'Unknown Dealership';
      } catch (e) {
    ...
    location: () => {
      try {
        return vehicle.dealership?.location || vehicle.location || vehicle.zipCode || 'Unknown Location';
      } catch (e) {
  ...
  const imageUrl = safeGet.imageUrl();
    ...
    >
      <div className="aspect-video bg-gray-200 relative">
        ...
        <div className="absolute top-2 right-2 bg-white py-1 px-2 rounded text-sm font-bold">
          {formatPrice(safeGet.price())}
        </div>
      </div>
      <div className="p-4">
        {/* Nova Autoland Promotion Banner */}
        {safeGet.dealershipName() === 'Nova Autoland' && (
          <div className="mb-3">
        ...
        <h3 className="font-semibold text-lg mb-2 truncate">{safeGet.title()}</h3>
        <div className="text-sm text-gray-700 space-y-2">
          <div className="flex items-center">
            <GaugeIcon className="w-4 h-4 mr-2 text-gray-500" />
            <span>{formatMileage(safeGet.mileage())}</span>
          </div>
          <div className="flex items-center">
            <BuildingIcon className="w-4 h-4 mr-2 text-gray-500" />
            <span>{safeGet.dealershipName()}</span>
          </div>
          <div className="flex items-center">
            <MapPinIcon className="w-4 h-4 mr-2 text-gray-500" />
            <span>{safeGet.location()}</span>
          </div>
